...
def Reconfig(Para,Info,Result_Planning,s):
    ...
    for n in range(Para.N_wind):
        ...
        model.addConstr(Var[N_C_wind + n] <= tp_wind[n])
    for n in range(Para.N_solar):
        model.addConstr(Var[N_S_solar + n] >= 0)
        model.addConstr(Var[N_S_solar + n] <= tp_solar[n])
...
#
def ReconfigRelax(Para,Info,Result_Planning,Result_Reconfig,s):
    #
    # minimize
    #       c * y + d
    # subject to 
...
if __name__ == "__main__":
    ...
    # Input parameter
    filename = "../data/Data-IEEE-24-demo.xlsx"
